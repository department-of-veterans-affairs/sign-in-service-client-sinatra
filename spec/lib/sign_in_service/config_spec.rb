...
RSpec.describe SignInService::Config do
  ...
  describe '#token_type' do
    it 'returns default token type' do
      expect(config.token_type).to eq(:cookie)
    end

    it 'can be set to bearer' do
      config.token_type = :bearer
      expect(config.token_type).to eq(:bearer)
    end

    it 'can not be set to an invalid value' do
      expect { config.token_type = :foo }.to raise_error(ArgumentError)
    end
  end
end
